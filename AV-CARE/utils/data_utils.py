...
def get_dataset_iemocap(data_folder: str, phase: str, img_interval: int) -> Dataset:
    ...
    meta_path = os.path.join(main_folder, 'meta - exc+hap.pkl')
    ...
    split_file = os.path.join(data_folder, 'IEMOCAP_SPLIT', f'{phase}_split.txt')
...
class IEMOCAP(Dataset):
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.ndarray, torch.Tensor, List[int], np.ndarray]:
        ...
        suffix = 'R' if use_left else 'L'
        audio_suffix = 'R' if use_left else 'L'
