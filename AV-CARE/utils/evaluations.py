...
def eval_iemocap(preds, truths, best_thresholds=None):
    ...
    cm = confusion_matrix(truths.argmax(-1), preds.argmax(-1), labels=[0, 1, 2, 3])
    ...
    colors = ["#ffffff", "#cad2c5", "#84a98c", "#52796f", "#246a73", "#2f3e46"]
    cmap = LinearSegmentedColormap.from_list("custom_cmap", colors, N=100)

    # === Plot the Normalized Confusion Matrix ===
    fig, ax = plt.subplots(figsize=(8, 6))
    disp = ConfusionMatrixDisplay(confusion_matrix=cm_normalized, display_labels=class_names)
    disp.plot(cmap=cmap, ax=ax, values_format=".2%", colorbar=True)

    plt.title('Confusion Matrix (Normalized by Row - Percentages)')
